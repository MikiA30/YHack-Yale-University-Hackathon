        ...
        {predictions.map((p) => (
          ...
          >
            <div className="flex items-start justify-between gap-2">
              ...
              <span
                className={`shrink-0 text-xs font-medium px-2 py-0.5 rounded-full border ${recBg[p.recommendation]}`}
              >
                <span className={recColor[p.recommendation]}>
                  {p.recommendation}
                </span>
              </span>
            </div>
            <div className="flex items-end gap-1">
              <span
                className={`text-3xl font-bold ${p.demand_change_pct >= 0 ? "text-emerald-400" : "text-red-400"}`}
              >
